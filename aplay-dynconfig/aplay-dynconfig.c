...
static snd_config_t **snd_get_all_dshare(snd_config_t *top, int *n_dshare)
{
	int err, i = 0;
	snd_config_t *res, *pcm, *entry, *type, **ds;
	...
	*n_dshare = 0;
	...
	err = snd_config_search(top, "pcm", &pcm);
	if (err) {
		printf("%s : Can't find pcm\n", __func__);
		return NULL;
	}

	snd_config_for_each(pos, next, pcm) {
		entry = snd_config_iterator_entry(pos);

		snd_config_search(entry, "slave.pcm.type", &type);
		if (!type)
			continue;

		snd_config_get_string(type, &str);
		if (strcmp(str, "dshare"))
			continue;
		(*n_dshare)++;
	}

	/* allocate array */
	ds = malloc(*n_dshare * sizeof(snd_config_t *));
	if (!ds)
		return NULL;
...
static void snd_print(snd_config_t *entry)
{
	const char *id;
	int ret;
	...
	snd_config_type_t t = snd_config_get_type(entry);
	switch(t) {
	case SND_CONFIG_TYPE_INTEGER: {
		long val;
		ret = snd_config_get_integer(entry, &val);
		if (ret)
			goto err;
		printf("- %s (int) %d\n", id, val);
		break;
	...
	case SND_CONFIG_TYPE_INTEGER64: {
		long long val;
		ret = snd_config_get_integer64(entry, &val);
		if (ret)
			goto err;
		printf("- %s (int64) %lld\n", id, val);
		break;
	...
	case SND_CONFIG_TYPE_REAL: {
		double val;
		ret = snd_config_get_real(entry, &val);
		if (ret)
			goto err;
		printf("- %s (real) %f\n", id, val);
		break;
	...
	case SND_CONFIG_TYPE_STRING: {
		const char *val;
		ret = snd_config_get_string(entry, &val);
		if (ret)
			goto err;
		printf("- %s (str) %s\n", id, val);
		break;
	...
	case SND_CONFIG_TYPE_POINTER: {
		const void *val;
		ret = snd_config_get_pointer(entry, &val);
		if (ret)
			goto err;
		printf("- %s (ptr) %p\n", id, val);
		break;
...
err:
	printf("%s : Cant get data\n", __func__);
}

static snd_config_t *snd_create_multi_plug(snd_config_t **dshares, int n_dshares)
{
	int i, n_bindings;
	char slave_id_str[2];
	char binding_id_str[2];

	snd_config_t *top;
	snd_config_t *node, *par, *slaves, *bindings;

	snd_config_make_compound(&top, "amp_all", 0);
	...
	par = node;
	snd_config_make_string(&node, "type");
	snd_config_set_string(node, "multi");
	snd_config_add(par, node);

	snd_config_make_compound(&slaves, "slaves", 0);
	slave_id_str[0] = 'a';
	slave_id_str[1] = '\0';
	for (i = 0; i < n_dshares; i++) {
		const char *pcm_id;
		snd_config_t *slave, *channels_node, *pcm_node;

		snd_config_make_compound(&slave, slave_id_str, 0);

		snd_config_make_integer(&channels_node, "channels");
		snd_config_set_integer(channels_node, 1);

		snd_config_get_id(dshares[i], &pcm_id);
		snd_config_make_string(&pcm_node, "pcm");
		snd_config_set_string(pcm_node, pcm_id);

		snd_config_add(slave, channels_node);
		snd_config_add(slave, pcm_node);
		snd_config_add(slaves, slave);

		slave_id_str[0] += 1;

	}
	snd_config_add(par, slaves);

	n_bindings = n_dshares * 1;

	snd_config_make_compound(&bindings, "bindings", 0);
	...
	for (i = 0; i < n_bindings; i++) {
		snd_config_t *binding, *slave_node, *channel_node;

		snd_config_make_compound(&binding, binding_id_str, 0);

		snd_config_make_string(&slave_node, "slave");
		snd_config_set_string(slave_node, slave_id_str);

		snd_config_make_integer(&channel_node, "channel");
		snd_config_set_integer(channel_node, 0);

		snd_config_add(binding, slave_node);
		snd_config_add(binding, channel_node);
		snd_config_add(bindings, binding);

		slave_id_str[0] += 1;
		binding_id_str[0] += 1;

	}
	snd_config_add(par, bindings);

	return top;
}

static void snd_config_walk(snd_config_t *cfg, int lv)
{
	snd_config_iterator_t pos, next;
	const char *val;
	const char *id;
	int i;
	snd_config_for_each(pos, next, cfg) {
		snd_config_t *entry = snd_config_iterator_entry(pos);

		for (i = 0; i < lv; i++)
			printf("%c", ' ');
		if (snd_config_get_type(entry) == SND_CONFIG_TYPE_COMPOUND) {
			snd_config_get_id(entry, &id);
			printf("%s\n", id);
			snd_config_walk(entry, lv + 1);
		} else {
...
int main(int argc, char *argv[])
{
	...
	err = snd_config_update_ref(&lconf);
	if (err < 0) {
		error(_("config loading error"));
		return 1;
	}

	snd_config_t **dshares;
	int n_dshares = 0;

	/* Get all the dshare plugins, used as slaves in the multi node */
	dshares = snd_get_all_dshare(lconf, &n_dshares);
	if (!dshares) {
		printf("%s : Can't get dshares list\n", __func__);
		return 1;
	}

	/* Create the multi plugin configuration */
	snd_config_t *multi;
	multi = snd_create_multi_plug(dshares, n_dshares);
	if (!multi) {
		printf("%s : Can't create multi plugin\n", __func__);
		return 1;
	}

	/* Insert the multi plugin under the 'pcm' node */
	snd_config_t *pcm;
	if (!snd_config_search(lconf, "pcm", &pcm)) {
		snd_config_add(pcm, multi);
	}

	/* Open with an already loaded configuration */
	err = snd_pcm_open_lconf(&handle, pcm_name, stream, open_mode, lconf);
	if (err < 0) {
